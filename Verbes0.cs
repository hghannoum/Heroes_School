...
namespace Start
{
    public partial class jeux_verbes : Form
    {
        ...
        string trueanswr;
            ...
            {
            foreach (RoundButton r in lblarr) r.BackColor = Color.Transparent; reponses = 0; score = 0;
            { panel1.Visible = true; panel2.Visible = false; }
            }
        ...
        { if (present.Checked || imaparfaitBtn.Checked || passeComp.Checked)
            ...
            if (reponses < 10)
            {
                ...
                try { if (choisi.Text == trueanswr) { score += 5; lblarr[reponses].BackColor = Color.Green; } else lblarr[reponses].BackColor = Color.Red; } catch { }; //AddTrue sounds  else add false sounds
                reponses++; NewConj();
            }
            else
            {
